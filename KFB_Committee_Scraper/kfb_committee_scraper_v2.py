# Health Check 실행
# python kfb_committee_scraper.py --check
...
# -------------------------------------------------
# HealthResult import (Health_Checker 쪽 공통)
# -------------------------------------------------
try:
    from common.health_result import HealthResult
except ImportError:
    # Health_Checker 외부 단독 실행 대비
    HealthResult = None


class KfbCommitteeScraper(BaseScraper):
    """은행연합회 규제심의위원회 결과 스크래퍼"""

    BASE_URL = "https://www.kfb.or.kr"
    LIST_URL = "https://www.kfb.or.kr/publicdata/reform_result.php"
    ...
    def extract_table_data(self, soup: BeautifulSoup) -> List[Dict]:
        ...
        for row in rows:
            ...
            # 첨부 파일 링크 (5번째 컬럼에서 추출)
            download_link = ""
            attachment_anchor = row.select_one("td:nth-child(5) > a")
            if attachment_anchor:
                href = attachment_anchor.get("href", "").strip()
                if href:
    ...
    def crawl_committee(
        ...
        seen_numbers = set()
        first_items = self.extract_table_data(soup)
        for item in first_items:
            ...
            if number and number not in seen_numbers:
                ...
                results.append(item)
        print(
            f"페이지 1/{total_pages} 완료: {len(first_items)}개 추출 "
            f"(누적: {len(results)}개)"
        )
        ...
        if total_pages > 1 and driver:
            ...
            for page_num in range(2, total_pages + 1):
                ...
                print(
                    ...
                    f"{len(new_items)}개 추출 (누적: {len(results)}개)"
                )
        ...
        for idx, item in enumerate(results, start=1):
            ...
            if not detail_link:
                print(f"[{idx}/{len(results)}] 상세 링크 없음 - {item.get('title','')}")
                item["content"] = ""
                item["department"] = ""
                item["file_name"] = ""
            ...
            print(f"[{idx}/{len(results)}] 상세 페이지 처리 중: {detail_link}")
            detail_soup = None

            if driver:
                detail_soup = self.fetch_detail_with_driver(driver, detail_link)
                print(f" driver 처리: {detail_link}")
...
def save_committee_results(records: List[Dict]):
    """JSON 및 CSV로 위원회 결과 데이터를 저장한다."""
    if not records:
        print("저장할 위원회 결과 데이터가 없습니다.")
        return
    ...
    json_path = os.path.join(json_dir, "kfb_committee_scraper.json")
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(
            {
                "crawled_at": time.strftime("%Y-%m-%d %H:%M:%S"),
                "url": KfbCommitteeScraper.LIST_URL,
    ...
    csv_path = os.path.join(csv_dir, "kfb_committee_scraper.csv")
...
def kfb_committee_health_check() -> Dict:
    ...
    try:
        ...
        try:
            ...
            prefs = {
                "download.default_directory": os.path.abspath(scraper.download_dir),
                "download.prompt_for_download": False,
                "download.directory_upgrade": True,
                "safebrowsing.enabled": True,
            }
        ...
        if driver:
            try:
                detail_soup = scraper.fetch_detail_with_driver(driver, detail_link)
            except Exception as exc:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="은행연합회 규제심의위원회 결과 스크래퍼")
    ...
    # -------------------------------------------------
    # Health Check 모드
    # python kfb_committee_scraper_v2.py --check
    # -------------------------------------------------
    if args.check:
        result = kfb_committee_health_check()
    ...
    # -------------------------------------------------        
    scraper = KfbCommitteeScraper()
    results = scraper.crawl_committee(limit=args.limit, download_files=not args.no_download)
