...
사용법:
    다른 스크래퍼에서 이 모듈을 사용하는 방법:

    1. 기본 사용 (비어있는 항목만 필터링):
       from Law_Scraper.excel_parser import LawExcelParser
       
       parser = LawExcelParser(excel_path="법규목록.xlsx")
       records = parser.parse_excel(filter_association_keyword='')
       parser.save_to_law_list_csv(records, output_path="Your_Scraper/input/list.csv")

    2. 특정 키워드로 필터링:
       # '협회 등' 컬럼에 '금융감독원'이 포함된 항목만 필터링
       records = parser.parse_excel(filter_association_keyword='금융감독원')
       parser.save_to_law_list_csv(records, output_path="Your_Scraper/input/list.csv")

    3. 다른 컬럼으로 필터링:
       # '자율규제' 컬럼이 비어있는 항목만 필터링
       records = parser.parse_excel(
           filter_association_keyword='',
           filter_column='자율규제'
       )
       parser.save_to_law_list_csv(records, output_path="Your_Scraper/input/list.csv")

    4. 필터링 없이 모든 데이터 추출:
       records = parser.parse_excel(filter_association_keyword=None)

    파라미터 설명:
        - filter_association_keyword: 
            * None: 필터링 안 함
            * '': 빈 문자열이면 해당 컬럼이 비어있는 항목만 필터링
            * '키워드': 특정 키워드가 포함된 항목만 필터링
    ...
    반환값:
        records: List[Dict] 형태로 다음 필드를 포함:
            - '법령명': 법령명
            - '법령유형': '법', '령', '행정규칙' 중 하나
            - '구분': '법령' 또는 '감독규정'
            - '검색_URL': 구분에 맞는 검색 URL
            - '약칭': 약칭 (참고용)

    저장 형식:
        save_to_law_list_csv()는 다음 형식의 CSV를 생성:
            구분,법령명
            법령,가상자산 이용자 보호 등에 관한 법률
            감독규정,개인금융채권의 관리 및 개인금융채무자의 보호에 관한 감독규정
"""
import sys
...
class LawExcelParser:
    ...
    def parse_excel(self, sheet_name: Optional[str] = None, header_row: Optional[int] = None, 
        ...
        try:
            ...
            # '법', '령', '행정규칙' 컬럼에서 값이 있는 모든 항목을 가져옴 (중복 제거 없음)
            records = []
            ...
            for idx, row in df.iterrows():
                ...
                # 검색 URL 결정
                search_url = search_urls.get(division, search_urls['법령'])  # 기본값: 법령 URL
                
                # 약칭 추출 (참고용)
                abbreviation = str(row.get('약칭', '')).strip()
                
                # '법', '령', '행정규칙' 컬럼에서 값이 있는 모든 항목을 레코드로 추가
                # 법 컬럼 확인
                law_value = str(row.get('법', '')).strip()
                if law_value and division:
                    record = {
                        '법령명': law_value,
                        '법령유형': '법',
                        '구분': division,
...
def main():
    ...
    # 데이터 파싱
    print("\n=== 데이터 파싱 시작 ===")
    # 기본값: '협회 등' 컬럼이 비어있는 항목만 필터링 (빈 문자열)
    ...
    if records:
        ...
        # 법령명 목록을 input/list.csv에 저장
        parser_obj.save_to_law_list_csv(records)
