...
def scrape_all(start_date=None, end_date=None):
    ...
    while True:
        ...
        # pagination 처리: 다음 페이지 존재 확인
        soup = BeautifulSoup(resp["text"], "html.parser")
        pagination_links = soup.select("div.pagination-set ul.pagination li a[href^='javascript:fnSearch']")
        next_page_index = None
        for a in pagination_links:
            page_num = a.get("data-pageindex")
            if page_num and int(page_num) > page_index:
                next_page_index = int(page_num)
                break
