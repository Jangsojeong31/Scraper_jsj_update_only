# 금융감독원 경영유의사항·개선권고 스크래핑 가이드

## 개요
금융감독원 경영유의사항·개선권고 공개 정보를 자동으로 스크래핑하여 JSON 및 CSV 파일로 저장하는 프로젝트입니다.
...
### 1. `fss_mngtnotice_scraper.py`
- 메인 스크래핑 스크립트
- 모든 페이지를 순회하며 데이터 수집
- PDF 다운로드 및 텍스트 추출 (V3 하이브리드 OCR 포함)
- 메타데이터 추출 (금융회사명, 제재조치일, 제재내용, 사건목록)
- 결과를 `output/fss_mngnt_result.json` 및 `output/fss_mngnt_result.csv`로 저장
...
- 전체 파이프라인 자동 실행 스크립트
- 실행 순서:
  1. `fss_mngtnotice_scraper.py`: 스크래핑 및 메타데이터 추출
  2. `post_process_ocr.py`: OCR 후처리 (새로 크롤링한 데이터)
  3. archive 병합: OCR 후처리된 데이터를 `archive/fss_mngnt_result.json`과 병합
...
# 기존 스크래핑 결과로 OCR 후처리만 다시 실행하고 싶을 때
...
외부망이 차단된 환경에서 본 스크립트를 운용할 때는 아래 항목을 사전에 준비해주세요. 단, 금융감독원 경영유의사항·개선권고 사이트(`https://www.fss.or.kr`)에 대한 접근만 허용되어 있다고 가정합니다.
...
6. **네트워크 화이트리스트 확인**
   - 금융감독원 경영유의사항·개선권고 공개 사이트(`https://www.fss.or.kr/fss/job/openInfoImpr/...`)에 대한 접근이 가능한지 보안팀과 협의합니다.
...
**자동 처리 과정:**
1. `fss_mngtnotice_scraper.py`: 전체 페이지 스크래핑, PDF 다운로드 및 텍스트 추출 (V3 하이브리드 OCR 포함), 메타데이터 추출 → `output/fss_mngnt_result.json` 저장
2. `post_process_ocr.py`: OCR 오류 자동 수정 및 품질 검증 (output 파일 처리)
3. archive 병합: OCR 후처리된 데이터를 `archive/fss_mngnt_result.json`과 병합하여 저장
...
또는 `fss_mngtnotice_scraper.py` 파일에서 직접 실행:
```bash
python fss_mngtnotice_scraper.py --sdate 2025-01-01 --edate 2025-12-31 --limit 10
```
...
### `archive/fss_mngnt_result.json`
- UTF-8 인코딩
- 전체 누적 데이터 (새로 크롤링한 데이터 + 기존 archive 데이터)
- 중복 제거 (제재조치일 + 금융회사명 기준)
- OCR 후처리 완료된 데이터

### `archive/fss_mngnt_result.csv`
- UTF-8-BOM 인코딩 (Excel 한글 호환)
- 전체 누적 데이터
- 엑셀에서 바로 열기 가능
...
### 신규 항목 추가 시
- [ ] `run_pipeline.py` 실행 (또는 `fss_mngtnotice_scraper.py` 실행)
- [ ] `output/fss_mngnt_result.json` 확인 (새로 크롤링한 데이터)
- [ ] `archive/fss_mngnt_result.json` 확인 (전체 누적 데이터)
- [ ] 결과 CSV 파일 확인
- [ ] 특이 케이스 수동 검토
...
### Archive 데이터 관리
- [ ] `archive/fss_mngnt_result.json`: 전체 누적 데이터 (백업 권장)
- [ ] `output/fss_mngnt_result.json`: 최신 스크래핑 결과만 (임시 파일)
...
- 메뉴: 업무안내 > 공시 > 경영유의사항·개선권고 공개

### 데이터 업데이트 주기
- 금융감독원은 부정기적으로 경영유의사항·개선권고를 공개
- 주기적으로 스크래핑하여 신규 항목 확인 필요
- `archive/fss_mngnt_result.json`에 전체 데이터가 누적되어 관리됨
...
- [x] 스크래핑 및 후처리 완료
- [x] 일반 패턴 인식 (경영유의, 개선사항)
- [x] OCR 텍스트 처리
- [x] 재심 케이스 처리
- [x] JSON/CSV 출력 (영문 키 사용)
- [x] 한글 인코딩 최적화
- [x] Archive 병합 기능
- [x] OCR 후처리 파이프라인 통합

**모든 패턴이 구현되어 신규 항목도 자동 처리 가능합니다!**
