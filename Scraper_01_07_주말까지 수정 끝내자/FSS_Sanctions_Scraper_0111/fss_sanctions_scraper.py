...
class FSSScraperV2:
    ...
    def save_results(self, filename='fss_sanc_result.json'):
        ...
        for item in split_results:
            ...
            for korean_key, english_key in column_mapping.items():
                json_item[english_key] = item.get(korean_key, '')
            # OCR추출여부는 그대로 유지
        ...
        try:
            ...
            if json_results:
                # 필드 순서: dvcv, srce, btcd, fnCompNm, tit, cntn, snctCntn, snctDt, atchFileUrl, atchFileNm, OCR추출여부
...
# Health Check 모드
# -------------------------------------------------
def fss_sanctions_check() -> dict:
    """
    금융감독원(FSS) 제재조치 현황 헬스체크
    - 목록 1건 추출
    - 상세 페이지 접근
    - 첨부파일 다운로드 가능 여부
    - PDF/OCR 본문 파싱 가능 여부
    ...
    try:
        ...
        result["checks"]["list_page"] = {
            "url": scraper.list_url_template.format(page=1, sdate='', edate=''),
            "success": True,
            "count": 1,
            "title": first_item.get('제재대상기관', '')
        }
        ...
        content, doc_type, file_url, filename = scraper.extract_attachment_content(detail_url, first_item.get('_link_text', ''))
        ...
        if file_url:
            try:
                head = scraper.session.head(file_url, timeout=15, allow_redirects=True)
...
if __name__ == "__main__":
    ...
    # -------------------------------------------------
    # Health Check 모드
    # python fss_scraper_v2.py --check
    # -------------------------------------------------
    if args.check:
        result = fss_sanctions_check()
