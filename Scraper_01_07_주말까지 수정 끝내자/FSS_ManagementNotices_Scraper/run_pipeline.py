...
def print_stats(json_path: Path) -> None:
    ...
    # fss_mngtnotice_scraper.py 구조에 맞게 필드명 확인
    # 제재내용 필드 확인 (영문 키와 한글 키 모두 지원)
    sanction_ok = sum(1 for item in data if has_value(item.get('snctCntn') or item.get('제재내용', '')))
...
def main() -> None:
    ...
    # archive 경로
    archive_path = ROOT_DIR / 'archive' / 'fss_mngnt_result.json'
    ...
    json_path = ROOT_DIR / 'output' / 'fss_mngnt_result.json'
    ...
    try:
        ...
        if not args.skip_scrape:
            ...
            run_step('fss_mngtnotice_scraper.py', '1. 목록 및 PDF 스크래핑', scraper_args if scraper_args else None)
            
            # OCR 후처리 실행 
            if json_path.exists():
                run_step('post_process_ocr.py', '2. OCR 후처리 (새로 크롤링한 데이터)')
            
            # OCR 후처리 완료 후 archive 병합
            if not args.no_merge and json_path.exists():
                ...
                try:
                    ...
                    try:
                        ...
                        for item in merged_data:
                            ...
                            for key in sorted(item.keys()):
                                if key.startswith('제목') or key == 'tit' or (key.startswith('tit') and len(key) > 3):
                                    ...
                                    content = item.get(content_key, item.get('cntn' if not idx else '내용', '')).strip()
                                    if title:
                                        titles.append(title)
                            ...
                            if titles:
                                ...
                                for title, content in zip(titles, contents):
                                    split_results.append({
                                        ...
                                        '출처': item.get('srce') or item.get('출처', '금융감독원'),
                                        ...
                                        '파일다운로드URL': item.get('atchFileUrl') or item.get('파일다운로드URL', ''),
                        ...
                        fieldnames = ['dvcv', 'srce', 'btcd', 'fnCompNm', 'tit', 'cntn', 'snctCntn', 'snctDt', 'atchFileUrl', 'atchFileNm', 'OCR추출여부']
            ...
            # skip_scrape 모드에서도 기존 output 데이터에 OCR 후처리 실행 가능
            if json_path.exists():
                run_step('post_process_ocr.py', '2. OCR 후처리 (기존 output 데이터)')
